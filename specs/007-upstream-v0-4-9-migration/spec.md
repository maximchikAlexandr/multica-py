...
**Feature Branch**: `upstream-v0-4-9-migration`
...
Every row is mandatory migration scope. Classification values are fixed in
`contracts/sdk-contract-v2.seed.json`; implementation does not assign them.

| Existing operation ID | Target behavior to prove | Minimum acceptance coverage |
| --- | --- | --- |
| `issues.create` | `issue create`; required nonblank title; inline/file/stdin description selector; existing priority/assignee/project; ordered post-create labels; no attachment expansion | Required title; each description channel; channel conflict; optional omission; ordered label steps and refresh |
...
| `issues.set_status` | Only `issue status <id> <status>`; remove stale `set-status`; no fallback/probe | Approved path success; legacy path absent; invalid/omitted status |
| `issues.comments.add` | `issue comment add <issue-id> --content <body>`; both values nonblank; no file/stdin/attachment expansion | Valid add; omitted/blank content and issue ID rejected |
...
| `issues.comments.list` | Four exact seed entrypoints: basic tuple; flat issue/since page; thread with `--tail` and optional atomic cursor pair; recent with positive default-10 limit and optional atomic cursor pair | Complete argv and return shape for all four; partial cursor, cursor-without-thread-limit, and bad ranges rejected |
...
| `projects.update` | Only name→`--title` and description→`--description`; `Unset` omits, empty emits clear, `description=None` rejects, all omitted rejects | Each presence state and complete argv |
| `projects.set_status` | Only `project status <id> <status>`; no legacy fallback/probe | Valid status; omitted/invalid status; legacy path absent |
...
| `projects.resources.add_local_directory` | Requires nonblank local path and daemon ID; fixes type to `local_directory`; `label=None` and blank label both omit `--ref-label` | Valid add; each required field absent; optional label None/blank omitted |
| `projects.resources.update_local_directory` | Exposes only a required nonblank local path; daemon and label are preserved by the target list-before-PUT flow; the SDK exposes no daemon/label update or clear | Valid path update; blank path rejected; preserved daemon/label verified |
...
`Deferred owner decision` is a terminal Feature-007 exclusion, not work or a
choice for its implementer. Those families remain unchanged and receive no
public SDK additions in this feature.
...
- The current CI produces coverage and mutation artifacts, but its five-stage
  baseline invocation does not require or pass offline JUnit, coverage JSON,
  and mutation results together. A green final baseline therefore does not yet
  prove all three required dimensions.

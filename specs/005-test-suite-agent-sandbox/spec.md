...
16. Multica-generated файлы `AGENTS.md` и `.multica/**` разрешены и не считаются неожиданными пользовательскими изменениями.
...
- **FR-044:** Filesystem manifest MUST разрешать изменения только `target.txt`, `AGENTS.md` и `.multica/**`; `control.txt` MUST быть byte-identical; другие created/removed/modified paths MUST падать.
...
- **SC-009:** Каждый deterministic workflow завершается ≤120 секунд; blocking live-smoke suite ≤300 секунд.
...
6. Multica пишет `AGENTS.md` и `.multica/project/resources.json` в local directory; эти paths входят в allowlist.

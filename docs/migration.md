...
`[0.4.42, 0.5.2)`. This is a direct additive model-compatibility update: task
runs may expose an optional opaque-string `wakeup_id`, and run messages may expose
an optional opaque-string `call_id`. Missing fields, including legacy payloads,
decode as `None`; present non-string values remain protocol errors.

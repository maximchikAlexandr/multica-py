...
class SquadMemberResource(BaseResource):
    ...
    def add_command(self, squad_id: str, member_id: str) -> Command[ActionResult[None]]:
        _ = cast("object", SQUAD_MEMBERS_ADD_BINDING)
        validate_nonblank(squad_id)
        validate_nonblank(member_id)
        return self._action_command(("squad", "member", "add", squad_id, member_id))

    def add(self, squad_id: str, member_id: str) -> ActionResult[None]:
        return self.add_command(squad_id, member_id).run()

    def remove_command(self, squad_id: str, member_id: str) -> Command[ActionResult[None]]:
        _ = cast("object", SQUAD_MEMBERS_REMOVE_BINDING)
        validate_nonblank(squad_id)
        validate_nonblank(member_id)
        return self._action_command(("squad", "member", "remove", squad_id, member_id))

    def remove(self, squad_id: str, member_id: str) -> ActionResult[None]:
        return self.remove_command(squad_id, member_id).run()

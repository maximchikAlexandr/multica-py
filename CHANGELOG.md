...
- Spawn/streaming timeouts raise `ProcessTimeoutError` (`CommandTimeoutError` and builtin `TimeoutError`) instead of bare `TimeoutError`; missing pipes raise `ProcessOutputCaptureError`; stream decode failures raise `EncodingError`

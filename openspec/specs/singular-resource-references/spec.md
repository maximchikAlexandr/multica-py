# singular-resource-references Specification

## Purpose

Defines typed `LazyRef[T]` singular resource references: passive handles with
explicit `get()`, cached `value`, atomic `refresh()`, and `invalidate()`
behavior, the normative inventory of supported singular edges, source-presence
semantics, loading lifecycle, errors, serialization passivity, and
bounded duplicate-aware prefetch integration.
...
`MulticaClient.prefetch()` SHALL accept selectors returning `LazyRef` as well as collection and mapping containers. Invocation-wide admission SHALL retain the existing rule that every source client shares the invoking client's process-semaphore object; a different semaphore SHALL raise `ValueError` before I/O, while other config differences SHALL remain admissible. Collection and mapping handles SHALL retain identity-only deduplication. Within an admitted invocation, singular references SHALL skip loaded handles, deduplicate identical handles, coalesce unloaded references only when exact originating scope, target service/type, and target ID all match, and publish independent bound target wrappers to each source handle. Equal targets under different full scopes SHALL be separate bounded jobs, not a validation error.

One private scope-key helper SHALL derive the exact singular coalescing scope from the effective normalized `ClientConfig` values that affect command execution or decoding: executable, server URL, profile, workspace ID, cwd, `tuple(config.environment)`, timeout, debug, encoding, compatibility policy, minimum CLI version, and maximum CLI version; it SHALL also include executor identity and process-semaphore identity. Path-like values SHALL use their post-`os.fspath` strings, environment SHALL preserve the exact execution order already stored by `ClientConfig` without re-sorting or deduplicating, timeout SHALL use its normalized `timedelta`, and URLs/identifiers/policies SHALL use their validated config values. Thus reversed tuples, including duplicate environment names whose last value wins, SHALL be different scopes. Display-only app URL and workspace slug SHALL NOT affect the key; `max_processes` SHALL be represented by the actual process-semaphore identity. Distinct target keys SHALL use the existing `ThreadPoolExecutor`, `max_parallel`, shared process semaphore, validation, and fail-fast rules.
...
- **THEN** they coalesce into one lookup; if any full-scope component other than the already-admitted semaphore differs, they run as separate bounded lookups without a validation error

#### Scenario: Environment execution order is part of singular scope
- **WHEN** equal target references use environment tuples `(("X", "b"), ("X", "a"))` and `(("X", "a"), ("X", "b"))` under the same process semaphore
- **THEN** they do not coalesce and run as separate bounded lookups because execution order and the last-value-wins result differ

...
- Add `TaskRun.stream_events()` for ordered incremental consumption from a bound task run, including cursor management, duplicate suppression, terminal-state detection, an ordinary-path completed/failed tail drain, and a documented cancellation quiescence drain.
...
- Document that the iterator is polling-backed incremental delivery, not server push or a real-time/completeness guarantee: externally cancelled runs expose no SDK-visible flush acknowledgement, and even completed/failed upstream drain is bounded and can discard failed persistence.

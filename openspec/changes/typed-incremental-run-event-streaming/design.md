...
`IssueResource.run_messages()` currently executes the `issue run-messages` operation retained in the approved Multica `v0.4.28` contract but decodes into a model whose `id`, `run_id`, and `role` fields do not exist in the tagged payload. The pinned Cobra command and user-authenticated handler already provide everything needed for incremental consumption: `--since <seq>`, ascending `seq`, semantic `type`, task/issue context, tool data, and timestamps. On current `main`, `TaskRun` remains a bound entity with inherited `issue_id`, a client, current status, cached raw `messages`, and a `messages_command()` relation; `IssueResource.runs(issue_id)` remains the available status refresh operation. The accepted `v0.4.28` work also made `IssueResource.run_messages()` explicitly return `Page[RunMessage]`, and current relation loaders already normalize that page through `_page_items`; this change extends those seams rather than restoring pre-upgrade tuple assumptions.
...
Add a private `_RunMessageWire` matching `taskMessageToPayload`, convert its recursive `input: object | None` through the same strict JSON coercion already used for autopilot payloads, and construct the public frozen `RunMessage`. Move the existing `_coerce_json_value` implementation verbatim from `entities/autopilots.py` to `multica_py/_internal/json_values.py`. Its only consumers SHALL be the existing `AutopilotRun` normalization paths in `entities/autopilots.py`, the existing autopilot wire conversions in `_internal/wire_models.py`, and the new run-message wire conversion there. Do not add a protocol, registry, wrapper class, or second coercion function. Generalize the existing private-wire decoder hook so `Page[RunMessage]` uses this adapter while other page decoding remains unchanged.
...
A private total conversion function maps exactly the five persisted `v0.4.28` categories: `text`, `thinking`, `tool_use`, `tool_result`, and `error`. Content maps to `text`, `thinking`, or `error` according to the concrete class. Hyphenated `tool-use`/`tool-result`, blank, and every other unrecognized `type` string map losslessly to `RunUnknownEvent(message_type=message.type)` and retain the complete raw message. At pinned Multica commit `38c992ad0a757434fb51584fa34e3bc57d1b78e1`, the persistence/drain implementation in `server/internal/daemon/daemon.go:7571-7903` writes the underscore spellings (`tool_use`/`tool_result` branches at `7761-7822`), while the public payload contract in `server/pkg/protocol/messages.go:163-174` documents only those persisted tool spellings; accepting internal pre-normalization aliases would add unsupported policy.
...
5. For `completed` or `failed`, immediately drain incremental reads until one response contains no unseen message; empty and verified identical-replay-only responses end the drain. No sleep is required between these ordinary-path reads. This captures rows successfully persisted by the upstream drain before the terminal callback, but it is not an upstream completeness watermark.
...
The split lifecycle follows pinned Multica commit `38c992ad0a757434fb51584fa34e3bc57d1b78e1`. `executeAndDrain` in `server/internal/daemon/daemon.go:7571-7930` normally flushes batches before `runTask` builds and returns its terminal result (`7070-7305`); `runner.run` then returns to the caller (`5080`), which reaches completed/failed reporting at `5121-5165` and `5410-5524`. The ordinary path therefore supports the immediate one-quiet-read drain. It is not absolute: a failed `ReportTaskMessages` is logged and discarded at `7686-7693`, and bounded `waitForDrain` can time out and “complete anyway” at `7860-7879`. The iterator cannot recover a row upstream never persisted, and a later raw snapshot is only a best-effort recovery for a row that was persisted after the iterator's quiet read. External cancellation differs further: `CancelTaskWithResult` in `server/internal/service/task.go:2351-2442` synchronously commits and broadcasts cancellation, while `server/internal/daemon/client.go:378-399`, the post-run cancellation-ack paths in `server/internal/daemon/daemon.go:5093-5117,5142-5162`, and `server/internal/handler/daemon.go:4330-4405` send/receive `AckTaskCancelled` only after the daemon has “finished flushing the transcript.” That ack is still not represented in `IssueResource.runs()`, so the SDK cannot wait for it. Two quiet reads reduce the cancellation race while preserving natural synchronous termination, but they do not guarantee capture of a message persisted after the quiet window. Future statuses terminal only by `completed_at` use the conservative cancellation path because no flush ordering is known. Documentation must state both limitations.
...
- **Terminal transcript persistence is not an absolute watermark** → Completed/failed use the source-backed ordinary one-quiet-read drain, but bounded upstream drain and failed message reporting can still omit a tail; cancelled/future terminals use two consecutive quiet reads separated by `poll_interval`. A later raw snapshot can recover only rows that were eventually persisted, never rows upstream discarded.

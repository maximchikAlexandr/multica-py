...
The split lifecycle follows pinned Multica commit `93342d04a7a9f788fec921e5aa736f86c7f22d8f`. `server/internal/daemon/daemon.go:6993-7030` flushes pending task messages before the completed/failed terminal callback, making the ordinary no-unseen drain evidence-backed. External cancellation differs: `server/internal/service/task.go:2219-2232` stores `cancelled` and `completed_at` synchronously, while `server/internal/daemon/client.go:340-346` and `server/internal/handler/daemon.go:3817-3828` send/receive `AckTaskCancelled` only after the daemon has “finished flushing the transcript.” That ack is not represented in `IssueResource.runs()`, so the SDK cannot wait for it. Two quiet reads reduce the cancellation race while preserving natural synchronous termination, but they do not guarantee capture of a message persisted after the quiet window. Future statuses terminal only by `completed_at` use the conservative cancellation path because no flush ordering is known. Documentation must state the limitation and direct consumers requiring authoritative cancelled-run completeness to fetch a later raw snapshot.

...
- [ ] 1.2 Add response catalog entries for `Page[T]` and compatible page subtypes for canonical CLI/direct-resource collection results, record the relation `.all()` tuple exception, and add `ActionResult[None]`, `ActionResult[str]`, `ActionResult[RepositoryMutationResult]`, and `ActionResult[RuntimeUpdateResult]`, keeping public types closed and free of `Any`.
...
- [ ] 4.6 Confirm `ProjectResourceCollection.update_local_directory` (`ProjectResourceUpdateLocalDirectoryRequest.local_path`) and `RuntimeResource.update` (`RuntimeUpdate.target_version`; `wait` remains an optional control field) preserve required-value semantics: omission or `None` is rejected before I/O, no all-omitted no-op is constructed, and required-value failures are covered separately from all-optional updates.
- [ ] 4.7 Add one frozen all-optional update-field presence table covering every applicable omitted, `None`, `""`, `()`, `False`, and `0` vector, plus a required-value boundary table for project-resource/runtime missing/null cases; assert exact preview and transport argv, result type, validation error, and multi-step ordering from approved contract data.
- [ ] 4.8 Add focused no-op tests only for the all-optional `ProjectUpdateRequest`, `AgentUpdateRequest`, `SkillUpdateRequest`, `IssueUpdateRequest`, `AutopilotUpdateRequest`, `LabelUpdateRequest`, `AutopilotTriggerUpdate`, and `UserProfileUpdate` surfaces, asserting the read command preview, one read execution, zero mutation execution, and current-entity return; explicitly exclude project-resource and runtime updates, whose omission/null failures are covered by 4.6.

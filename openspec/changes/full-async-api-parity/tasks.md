...
- [ ] 1.2 Add one thread-safe per-process lifecycle state machine in `src/multica_py/process.py` with short mutex sections for admission, an in-flight provider-operation lease count, buffered/streaming and collection ownership, active stdout/stderr registrations, `open`/`collecting`/`closing`/`finalized` transitions, published outcomes, and exactly-once handle-close/cleanup/semaphore-release; acquire/release leases around poll/collect/stream-read/wait/terminate/kill and run provider I/O and cleanup outside the mutex.
...
- [ ] 1.4 Implement explicit close admission: block ordinary new leases, admit only the close-owned terminate/wait/kill sequence, revoke paused stream generators, wait for already leased stream reads/provider calls, publish one close outcome, and return only after handle close, cleanup, and semaphore release; later revoked iteration SHALL end without provider I/O.
- [ ] 1.5 Add event/barrier-driven tests for every sync/async pairing of poll with result/close and terminate/kill with result/stream finalization on local, SSH, and microsandbox handles; assert admitted leases finish before finalization, post-close calls perform no provider I/O, and both callers complete with the specified cached/no-op/error outcome.
- [ ] 1.6 Add deterministic same-thread partial-stream then close, abandoned generator, concurrent stdout+stderr reads, sync/async context exit, close-close, and cancelled-await tests; assert no self-deadlock, close returns only after finalization, revoked iteration stops, passive finalization waits both streams, and each finalization side effect occurs once.
...
- [ ] 1.8 Run the focused command/process tests plus `uv run mypy src` and fix until the new generic and lifecycle annotations are precise without public `Any`.
...
- [ ] 5.5 Update the mandatory completeness counters in repository `AGENTS.md` from the stale 193 canonical / 320 cases / 231 manual rows to the canonical consumer's existing 194 / 321 / 232 values, without changing `OPERATION_CASES`, sync discovery, approved upstream contract/descriptors, or adding `_async` symbols to the sync inventory.

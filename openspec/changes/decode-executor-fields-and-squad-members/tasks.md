...
- [ ] 1.1 Add `AgentSkill(msgspec.Struct, frozen=True, kw_only=True)` with fields `id: str`, `name: str`, `enabled: bool` to `src/multica_py/models/agents.py` (before `class Agent`).
- [ ] 1.2 In `src/multica_py/models/agents.py`, change `Agent.skills` from `tuple[str, ...] = ()` to `tuple[AgentSkill, ...] = ()`.
- [ ] 1.3 In `src/multica_py/models/agents.py`, add `archived_at: datetime.datetime | None = None` to `Agent` (after `skills`). `import datetime` is already present.

## 2. Squad model: leader_id and archived_at

- [ ] 2.1 In `src/multica_py/models/system.py`, add `import datetime` at the top (currently only `import msgspec`).
- [ ] 2.2 In `src/multica_py/models/system.py`, add `leader_id: str | None = None` and `archived_at: datetime.datetime | None = None` to `Squad` (after `member_count: int = 0`). Add a `ponytail:` comment on `Squad` noting `member_type`/`role` enums are deferred until upstream values stabilise (applies to `SquadMember` in task 3).

## 3. SquadMember model and members resource

- [ ] 3.1 Add `SquadMember(msgspec.Struct, frozen=True, kw_only=True)` with `member_id: str`, `member_type: str`, `role: str` to `src/multica_py/models/system.py` (after `Squad`). Add a `# ponytail: member_type/role are free str, no enum — upstream values not stabilised; add enums when they are` comment.
- [ ] 3.2 Create `src/multica_py/resources/squad_members.py` with `SquadMemberResource(BaseResource)` whose `list(self, squad_id: str) -> tuple[SquadMember, ...]` calls `self._run_json_decode_list(("squad", "member", "list", squad_id), SquadMember)`. Import `SquadMember` from `multica_py.models.system`.
- [ ] 3.3 In `src/multica_py/resources/squads.py`, import `SquadMemberResource` and set `self.members = SquadMemberResource(transport, config)` in `SquadResource.__init__` (mirror `AgentResource.skills` at `src/multica_py/resources/agents.py:15`).

## 4. AgentSkillResource returns typed AgentSkill

- [ ] 4.1 In `src/multica_py/resources/agent_skills.py`, change the `list` return type from `tuple[Skill, ...]` to `tuple[AgentSkill, ...]` and change the `_run_json_decode_list` item type from `Skill` to `AgentSkill`. Import `AgentSkill` from `multica_py.models.agents` (replace the `Skill` import). The `set` method is unchanged.

## 5. Operation cases

- [ ] 5.1 In `tests/cases/operations.py`:
...
- [ ] 5.2 Update `test_discovered_public_methods` in `tests/unit/resources/test_operations.py`:
...
- [ ] 6.2 Run `uv run pytest tests/unit/resources/test_operations.py::test_legacy_payload_bijection -v` and confirm it passes. If any other legacy row fails, its fingerprint also drifted — regenerate it too and note which row in the run.

## 7. Decoder contract tests

- [ ] 7.1 Create `tests/contract/test_executor_models.py` with `pytestmark = [pytest.mark.contract]` (or rely on the `tests/contract/` path marker auto-applied by `tests/conftest.py`). Add two `@pytest.mark.parametrize` tests:
...
- [ ] 8.1 `uv run pytest -m "not live"` green.
- [ ] 8.2 `uv run mypy src` and `uv run mypy tests` green (no `Any` leaks; `AgentSkill`/`SquadMember` are typed structs).
- [ ] 8.3 `uv run ruff check .` and `uv run ruff format --check .` green.
- [ ] 8.4 `uv run pytest tests/unit/resources/test_operations.py::test_discovered_public_methods -v` asserts 117 canonical methods and 141 total cases.
- [ ] 8.5 `uv run pytest tests/unit/resources/test_operations.py::test_legacy_payload_bijection -v` passes with the 8 regenerated fingerprints.
- [ ] 8.6 `uv run pytest tests/contract/test_executor_models.py -v` passes all decoder rows.
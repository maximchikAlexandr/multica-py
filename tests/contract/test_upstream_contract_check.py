...
def test_check_exits_zero_for_clean_fixture(
...
def test_check_check_without_output_writes_nothing(
...
def test_check_check_with_output_writes_report(
    tmp_path: pathlib.Path,
    contract_cli: ContractCliRunner,
) -> None:
    target = tmp_path / "report.json"
    result = contract_cli.run("check", "--format", "json", "--check", output=target)
    assert result.returncode == 0
    data = cast("dict[str, object]", json.loads(target.read_text()))
    assert data["status"] == "clean"


def test_check_json_output_writes_file(
    tmp_path: pathlib.Path,
    contract_cli: ContractCliRunner,
) -> None:
    target = tmp_path / "report.json"
...
def test_check_offline_does_not_touch_network(
    contract_cli: ContractCliRunner,
) -> None:
    result = contract_cli.run("check", "--format", "human")
...
def test_check_runs_under_30_seconds(
    contract_cli: ContractCliRunner,
) -> None:
...
def test_check_exit_code_for_invalid_artifact(
    tmp_path: pathlib.Path,
    contract_cli: ContractCliRunner,
) -> None:
    fake_root = tmp_path
...
def test_check_with_candidate_missing_file_exits_three(
    ...
    state = cast("dict[str, object]", json.loads(state_path.read_text(encoding="utf-8")))
    state["candidate"] = {
        "version": "0.4.3",
        "tag": "v0.4.3",
    ...
    result = contract_cli.run("check", "--format", "human", "--with-candidate")
    assert result.returncode == 3


def test_check_clean_when_candidate_present_without_with_candidate(
    preserved_generated_state: None,
    contract_cli: ContractCliRunner,
) -> None:
    state_path = ROOT / "src" / "multica_py" / "_generated" / "upstream_state.json"

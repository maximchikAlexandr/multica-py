...
def _collect_paginated_issue_ids(
    ...
    *,
    ...
    pages = 0
    while pages < 10:
        items, next_cursor = live_session.oracle.list_issues_page(
            limit=limit,
            cursor=cursor,
            status=status,
            label_id=label_id,
...
def test_issue_pagination_collects_twelve_issues_without_duplicates(
    ...
    for index in range(ISSUE_COUNT):
        issue = live_session.client.issues.create(
            IssueCreateRequest(title=f"{live_case.unique_name}-page-{index:02d}")
        )
        created_ids.append(issue.id)
        live_case.defer_cleanup(
            live_session.oracle.delete_callback(f"/api/issues/{issue.id}", "issue"),
        )
    collected = _collect_paginated_issue_ids(live_session, limit=PAGE_SIZE)
...
def test_issue_filter_status_and_label_id(
    ...
    target_label = live_session.client.labels.create(
        label_name(live_case.unique_name, "flt"), color="#445566"
    )
    other_label = live_session.client.labels.create(
        label_name(live_case.unique_name, "oth"), color="#667788"
    )
    for label in (target_label, other_label):
        live_case.defer_cleanup(
            live_session.oracle.delete_callback(f"/api/labels/{label.id}", "label"),
        )
    matching = live_session.client.issues.create(
        IssueCreateRequest(
            title=f"{live_case.unique_name}-match",
            label_ids=(target_label.id,),
        )
    )
    non_matching_status = live_session.client.issues.create(
        IssueCreateRequest(
            title=f"{live_case.unique_name}-wrong-status", label_ids=(target_label.id,)
        )
    )
    non_matching_label = live_session.client.issues.create(
        IssueCreateRequest(
            title=f"{live_case.unique_name}-wrong-label", label_ids=(other_label.id,)
        )
    )
    ...
    live_session.client.issues.set_status(matching.id, IssueStatus.in_progress)
    ...
    sdk_filtered = live_session.client.issues.list(IssueListFilter(status=IssueStatus.in_progress))

...
def test_plugin_init_uses_text_action_result_without_unsupported_output_flag() -> None:
    transport = MagicMock(spec=CliTransport)
    transport.build_full_argv.side_effect = lambda args: ("multica", *args)
    transport.run_text.return_value = TextResult("Initialized Private Plugin in /tmp/plugin", "", 0)
    resource = PluginResource(transport, ClientConfig())

    result = resource.init("/tmp/plugin")

    transport.run_text.assert_called_once_with(("plugin", "init", "/tmp/plugin"))
    assert result.value is None
    assert result.message == "Initialized Private Plugin in /tmp/plugin"


def test_plugin_list_and_status_accept_explicit_workspace_overrides() -> None:
    resource = PluginResource(CliTransport(ClientConfig()), ClientConfig())

    assert resource.list_command(workspace="workspace-a")._plan.steps[0].argv == (
        "plugin",
        "list",
        "--workspace",
        "workspace-a",
    ...
    assert resource.status_command("plugin-a", workspace="workspace-b")._plan.steps[0].argv == (
        "plugin",
        "status",
        "plugin-a",
        "--workspace",
...
def test_remote_mcp_configure_passes_public_config_path_without_reading_it(
    tmp_path: pathlib.Path,
    mock_transport: MagicMock,
    raw_result: Callable[..., RawCommandResult],
) -> None:
    public_config = tmp_path / "public-config.json"
    resource = PluginResource(mock_transport, ClientConfig())
    command = resource.configure_remote_mcp_command(
        "inst_001",
        "remote-mcp",
        endpoint="https://mcp.example.com",
        public_config_file=public_config,
    )
    ...
    mock_transport.run_bytes.assert_called_once_with(
        command._plan.steps[0].argv, stdin=None, timeout=None
    )

    preview = PluginResource(
        CliTransport(ClientConfig()), ClientConfig()
    ).configure_remote_mcp_command(
        "inst_001",
        "remote-mcp",
        endpoint="https://mcp.example.com",
        public_config_file=public_config,
    )
    assert str(public_config) in preview.commands[0]
